...
def MainMenu():
    ...
    if showByRecordingGroup:
        dir.add(
            DirectoryObject(
                key=Callback(GroupRecordingsBy, groupByList=['Recording/RecGroup'], staticBackground=BY_RECGROUP_BACKGROUND), 
                title=L2('BY_RECORDING_GROUP'),
                thumb=R(BY_RECGROUP_ICON)
            )
        )
